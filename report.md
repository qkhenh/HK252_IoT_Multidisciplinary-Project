...
| **security_guards** | Bảo vệ | user_id (PK/FK), **assigned_lane_id (FK → lanes)**, employee_code, shift_start, shift_end |
...
| Table | Description | Columns |
|-------|-------------|---------|
| **access_logs** | Log ra vào | log_id, **lane_id (FK)**, vehicle_id, guest_reg_id, **token_id (FK → access_tokens)**, guard_id, check_in_time, **detected_text** *(chuỗi OCR thô: biển số / OTP 6 số / UUID QR)*, image_snapshot_data (BYTEA), access_method, is_access_granted, **action_reason** *(lý do mở thủ công — UC-07)* |
