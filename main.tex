...
\fancyfoot[L]{\scriptsize \ttfamily IoT Multidisciplinary Project - Academic year 2024-2025}
...
\begin{itemize}
    ...
    \item \textbf{IP Camera:} Hardware device that captures vehicle images at the gate, which also reads ePass-style QR codes directly via software libraries.
    \item \textbf{QR Scanner:} 1-way physical scanner placed at a low level for residents (pedestrians/cyclists) to scan their personal dynamic QR code via the Mobile App, sending data to the Backend via HTTP POST.
...
\begin{itemize}
    ...
    \item \textbf{AI Service:} Python-based service using YOLOv8 for plate detection and PaddleOCR for text extraction, along with QR scanning libraries for reading ePass on windshields.
    \item \textbf{IoT Gateway:} Python application bridging the backend with Arduino hardware for gate control.
    \item \textbf{Hardware Layer:} Arduino, servo motor, IR sensors, IP camera, and a separate Hardware QR Scanner used exclusively for pedestrians/cyclists to read personal dynamic QR codes from their smartphones.
\end{itemize}
...
\begin{longtable}{@{}
    ...
    \addlinespace
    Guest Registration & \xmark & \xmark & \cmark (Full) \\
    \addlinespace
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item If FOUND: The system starts the 2-layer security check:
        \begin{itemize}
            \item \textbf{QR Anti-Spoofing Check:} Does the \texttt{qr\_tag\_text} sent by the AI match the \texttt{vehicle\_qr\_tag} in the Database? (If the AI sends null or a mismatched string $\rightarrow$ Block immediately, issue a `Suspected fake vehicle'' red alert).
            \item \textbf{Anti-Passback Check:} The system verifies the vehicle's direction (inbound) against its state in the database. If it is already \texttt{inside} $\rightarrow$ Logic error $\rightarrow$ Block immediately, alert `Vehicle already inside''.
        \end{itemize}
        \item \textbf{Approval:} If all security tests pass $\rightarrow$ Change \texttt{location\_status} to \texttt{inside} $\rightarrow$ Save log $\rightarrow$ Open the gate. Flow ends.
    \end{itemize}
...
\begin{tcolorbox}[colback=white!95!gray, colframe=black, title=Scenario 3: Multi-step Exceptions (OTP-based Guest Access)]
\begin{enumerate}
    \item This flow is designed for unexpected guests (walk-ins) or delivery drivers.
    \item The guest arrives at the gate and is blocked because they are not registered.
    \item The guest calls the resident to inform them of their arrival.
    \item The resident uses the mobile app to register the guest and generate a new OTP.
    \item The generated OTP is sent to the resident AND instantly displayed on the Security Guard's dashboard monitor.
    \item The resident communicates the OTP to the guest via phone call or message.
    \item The guest reads the OTP to the Security Guard.
    \item The Guard verifies the OTP matching on their screen and triggers the gate opening.
\end{enumerate}
\end{tcolorbox}
...
\begin{enumerate}
    ...
    \item The resident places their smartphone against the Hardware QR Scanner located at a lower level at the gate.
    \item The scanner sends the UUID string to the Backend via HTTP POST. The Backend retrieves the resident's data and verifies it $\rightarrow$ Opens the gate.
\end{enumerate}
\end{tcolorbox}
...
\begin{itemize}
    ...
    \item \textbf{Authentication Tokens:} Stores short-lived OTP codes (5-minute validity) and digital QR codes for manual or non-motorized entry.
    ...
    \item \textbf{AI Metadata:} Stores confidence scores of plate recognition and high-dimensional vectors for advanced image similarity searches.
...
Description & Citizen views and adds vehicles to personal whitelist for automatic gate access.\\ \hline
Preconditions &
...
\begin{itemize}[leftmargin=*, nosep, topsep=0pt, partopsep=0pt, parsep=0pt, after=\strut]
    \item \textbf{Success:} New vehicle is added to system with is\_active = false (waiting for approval).
    \item \textbf{Failure:} Error message displayed, vehicle not added.
\end{itemize}
\\ \hline
Basic Flow & 
\begin{enumerate}[leftmargin=*, nosep, topsep=0pt, partopsep=0pt, parsep=0pt]
    ...
    \item Citizen selects Add New Vehicle
    \item Citizen enters: License Plate, Vehicle Type, Color, Image (optional)
    \item System validates license plate format and checks for duplicates
    \item System saves vehicle with is\_active = false (pending approval)
...
Description & When vehicle approaches gate, IoT Gateway captures license plate image using camera and sends to AI Service; AI Service uses YOLOv8 model to detect license plate region and applies OCR to extract plate number; System compares extracted plate with registered vehicles in database; If plate matches authorized vehicle, system automatically opens gate barrier; All access attempts are logged with timestamp, confidence score, and captured image for audit purposes \\ \hline
Preconditions &
\begin{itemize}[leftmargin=*, nosep, topsep=0pt, partopsep=0pt, parsep=0pt]
    \item Camera and IR sensor at gate are operational
    \item IoT Gateway is running and connected to backend
    \item AI Service with YOLOv8, OCR model, and QR scanning library is available
    \item Separate QR Scanner for pedestrians is operational
...
Postconditions &
\begin{itemize}[leftmargin=*, nosep, topsep=0pt, partopsep=0pt, parsep=0pt, after=\strut]
    \item \textbf{Success:} Vehicle plate recognized with high confidence and matches ePass QR data read from camera (for residents) or guest list. Gate opens automatically. System logs timestamp and toggles \texttt{is\_inside}.
    \item \textbf{Failure:} Plate not recognized, ePass QR spoofing detected, or confidence too low. Gate remains closed. Guard alerted.
\end{itemize}
\\ \hline
Basic Flow & 
\begin{enumerate}[leftmargin=*, nosep, topsep=0pt, partopsep=0pt, parsep=0pt]
    ...
    \item IoT Gateway sends command to capture image from IP camera
    \item Camera captures high-resolution image of vehicle front (including license plate and ePass QR on windshield)
    \item IoT Gateway sends captured image to AI Service via HTTP API
    \item AI Service preprocesses image (resize, normalize, enhance contrast)
    \item AI Service runs YOLOv8 model to detect license plate bounding box AND uses software library to scan the ePass QR code in the image
    \item AI Service applies OCR (PaddleOCR) to extract text from plate
    \item AI Service returns extracted plate number, confidence score, and extracted ePass QR data
    \item IoT Gateway sends plate number and ePass QR data to Backend API for verification
    \item Backend verifies the matched license plate and valid ePass QR code to confirm anti-spoofing
    \item Backend checks logical flow based on \texttt{is\_inside} flag
    \item Backend creates access\_log entry with access\_type = 'ai\_checkin'
...
\\ \hline
Alternative Flow &
\noindent \textbf{AF1: Guest with valid access token (at step 12)}
\begin{enumerate}
    \item Backend does not find plate in vehicles table
    \item Backend queries guest\_registrations and access\_tokens tables
    \item Backend finds valid guest registration with matching expected plate
    \item Backend verifies access token is within valid\_from and valid\_until
    \item Backend authorizes entry and logs as 'guest\_checkin'
\end{enumerate}
...
The IoT Gateway is a Python application (\texttt{SystemCommunication.py}) that serves as the bridge between the Backend server and the Arduino hardware. It connects to the Backend via WebSocket (Socket.IO) and communicates with Arduino via PySerial to control the gate barrier.
...
The following tables define the serial protocol between the IoT Gateway (\texttt{SystemCommunication.py}) and the Arduino (\texttt{Hardware.ino}) operating at 9600 baud.
