...
\begin{enumerate}
    ...
    \item The resident opens the Mobile App, selects ``Create OTP'' — the system generates a random 6-digit code (valid for 3 minutes) and displays it on the app.
...
\begin{longtable}{@{}
    ...
    \addlinespace
    FR-07 & The system shall allow citizens to create emergency OTP codes (6 digits, valid for 3 minutes). & \cmark & \cmark & High \\
    \addlinespace
    ...
    \addlinespace
    FR-15 & The system shall provide managers with a dashboard showing 4 KPIs: Total Traffic Volume, Automation Rate, Security Alerts, and Vehicles Inside. & \cmark & \cmark & High \\
    \addlinespace
    FR-16 & The system shall allow managers to manage system user accounts (list, create, update, delete). & \cmark & \cmark & Medium \\
\end{longtable}
...
\begin{longtable}{@{}
    ...
    \addlinespace
    NFR-10 & The OTP codes shall be 6 digits, unique, and expire after 3 minutes. & \cmark & \cmark & High \\
\end{longtable}
...
\begin{itemize}
    ...
    \item \textbf{Authentication Tokens:} Stores short-lived OTP codes (3-minute validity) and dynamic QR codes for non-motorized entry (pedestrians/cyclists).
...
Postconditions &
\begin{itemize}[leftmargin=*, nosep, topsep=0pt, partopsep=0pt, parsep=0pt, after=\strut]
    \item \textbf{Success:} OTP code created with 3-minute validity period. OTP is displayed to Citizen and synced to Guard's monitor.
...
\begin{enumerate}[leftmargin=*, nosep, topsep=0pt, partopsep=0pt, parsep=0pt]
    ...
    \item System saves OTP to access\_tokens with valid\_until = NOW() + 3 minutes
...
\begin{longtable}{@{}
    ...
    \addlinespace
    4 & Vehicles Inside & Number of resident vehicles currently inside the compound (real-time count). & \texttt{COUNT(*) FROM vehicles WHERE is\_inside = true AND is\_active = true} with zone matching. \\
\end{longtable}
...
\begin{enumerate}
    ...
    \begin{itemize}
        \item On \texttt{OPEN}: sends \texttt{ENTRY\_GO:OwnerName\textbackslash n} (entry lane) or \texttt{EXIT\_GO:OwnerName\textbackslash n} (exit lane) to Arduino $\rightarrow$ Arduino opens barrier and shows welcome message on LCD.
        \item On denial: sends \texttt{DENY\_PLATE:PlateText\textbackslash n} $\rightarrow$ Arduino shows \texttt{UNAUTHORIZED} on LCD.
    ...
    \begin{itemize}
        ...
        \item Listens for \texttt{manual\_command} event from Backend: \texttt{OPEN} $\rightarrow$ sends \texttt{MANUAL\_OPEN\_IN} or \texttt{MANUAL\_OPEN\_OUT} (by lane); \texttt{CLOSE} $\rightarrow$ sends \texttt{FORCE\_CLOSE\textbackslash n}.
    \end{itemize}
\end{enumerate}
\end{tcolorbox}
...
\begin{longtable}{@{}
    ...
    \texttt{ENTRY\_GO:\textit{OwnerName}\textbackslash n} & Open entry barrier (Lane A); Arduino displays welcome message on LCD and beeps once. \\
    \addlinespace
    \texttt{EXIT\_GO:\textit{OwnerName}\textbackslash n} & Open exit barrier (Lane B); Arduino displays \texttt{SAFE TRAVELS} on LCD and beeps once. \\
    \addlinespace
    \texttt{MANUAL\_OPEN\_IN:\textit{Operator}\textbackslash n} & Manually open entry barrier; barrier stays open (no auto-close). \\
    \addlinespace
    \texttt{MANUAL\_OPEN\_OUT:\textit{Operator}\textbackslash n} & Manually open exit barrier; barrier stays open (no auto-close). \\
    \addlinespace
    \texttt{FORCE\_CLOSE\textbackslash n} & Emergency close all barriers; Arduino displays \texttt{EMERGENCY CLOSED} on LCD. \\
    \addlinespace
    \texttt{DENY\_PLATE:\textit{PlateText}\textbackslash n} & Access denied; Arduino displays \texttt{UNAUTHORIZED} on LCD and beeps once. \\
    \addlinespace
    \texttt{ALARM\_FAKE:\textit{Info}\textbackslash n} & Anti-passback alert; buzzer sounds 5 times and all barriers close. \\
\end{longtable}
...
\begin{longtable}{@{}
    ...
    \texttt{CAR\_ARRIVED\textbackslash n} & HC-SR04 sensor (Lane A) detected a vehicle at the entry gate; triggers AI license plate recognition in the Gateway. \\
    \addlinespace
    \texttt{CAR\_EXITING\textbackslash n} & HC-SR04 sensor (Lane B) detected a vehicle at the exit gate; triggers AI license plate recognition in the Gateway. \\
\end{longtable}
...
\begin{itemize}
    ...
    \item \textbf{VEHICLES} (vehicle\_id, owner\_user\_id, vehicle\_type, license\_plate, vehicle\_color, is\_inside, is\_active, status, pending\_changes, last\_log\_time, registered\_at)
    \begin{itemize}
...
This project successfully designed and implemented a \textbf{Smart Toll Gate System} — an automated vehicle access control solution for residential areas, integrating IoT hardware, AI-based recognition, and a full-stack web platform.

The delivered system covers all three user roles end-to-end:
\begin{itemize}
    \item \textbf{Citizens} can register vehicles, pre-register guests with scheduled visit windows, generate 6-digit OTP codes (3-minute validity), and create personal QR tokens for non-motorized entry — all via a web dashboard.
    \item \textbf{Security Guards} monitor gate activities in real-time through a WebSocket-powered dashboard, perform manual gate overrides with mandatory reason selection, and submit AI plate correction feedback.
    \item \textbf{Managers} view a 4-KPI operational dashboard (Total Traffic, Automation Rate, Security Alerts, Vehicles Inside), approve vehicle registration requests, and manage user accounts within their zone.
\end{itemize}

On the hardware side, the IoT Gateway (\texttt{TestingMode.py}) orchestrates two lanes simultaneously: it scans for QR codes first, then classifies AI output as either a 6-digit OTP or a license plate, and communicates with the Arduino via a structured serial protocol.

\subsection*{Key Technical Contributions}

\begin{itemize}
    \item \textbf{Anti-Passback:} The \texttt{is\_inside} flag on each vehicle record prevents a vehicle from entering twice without exiting, enforced at the database query level.
    \item \textbf{Dual-Lane Serial Protocol:} Seven outbound commands (\texttt{ENTRY\_GO}, \texttt{EXIT\_GO}, \texttt{MANUAL\_OPEN\_IN/OUT}, \texttt{FORCE\_CLOSE}, \texttt{DENY\_PLATE}, \texttt{ALARM\_FAKE}) and two inbound signals (\texttt{CAR\_ARRIVED}, \texttt{CAR\_EXITING}) give precise, lane-specific hardware control. A \texttt{manualOverride} flag prevents accidental auto-close during manual operations.
    \item \textbf{OCR Post-Processing:} A character correction table (e.g., \texttt{O→0}, \texttt{I→1}, \texttt{S→5}) disambiguates AI output — 6 corrected digits are treated as OTP; otherwise as a license plate — reducing guard intervention.
    \item \textbf{Unicode-Safe LCD:} \texttt{format\_lcd\_text()} removes Vietnamese diacritics and non-ASCII characters before sending to the 16×2 LCD via Serial, preventing garbled hardware output.
\end{itemize}

\subsection*{Limitations}

\begin{itemize}
    \item \textbf{Lighting and Angle Sensitivity:} YOLOv8 + PaddleOCR recognition degrades under poor lighting or steep camera angles. A fixed camera mount and adequate lighting are prerequisites for reliable production use.
    \item \textbf{Network Dependency:} The IoT Gateway has no offline fallback. If the connection to the Backend is interrupted, the gate falls back to full manual operation.
    \item \textbf{Single-Gate Testing:} Integration testing was conducted on one physical gate. Multi-gate, multi-zone concurrency has not been validated under real load.
\end{itemize}

\subsection*{Future Work}

\begin{itemize}
    \item \textbf{Improved OCR Accuracy:} Fine-tune the YOLOv8 model on a Vietnamese license plate dataset to reduce misreads under real-world lighting conditions.
    \item \textbf{Mobile Companion:} Extend the citizen interface to a mobile app so residents can generate OTP/QR tokens and receive gate-event notifications without opening a browser.
    \item \textbf{Multi-Gate Deployment:} Deploy and test the system across multiple gates in the same zone to validate the real-time WebSocket broadcast under concurrent load.
\end{itemize}
...
The source code for this project is hosted on GitHub. You can access it via the following link:

\begin{center}
    \url{https://github.com/HuyViete/DE_Project.git}
\end{center}
