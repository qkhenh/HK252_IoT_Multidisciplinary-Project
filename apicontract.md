...
```json
{
  ...
  "data": {
    ...
    "full_name": "string",
    "access_token": "eyJ...",
    "citizen_details": { "zone_id": 1, "zone_name": "Khu A", "address": "A-101", "phone_number": "...", "identity_card_number": "...", "is_house_owner": true },
    "guard_details": { "assigned_gate_id": 1, "assigned_gate_name": "Cổng chính", "employee_code": "...", "shift_start": "...", "shift_end": "..." },
    "manager_details": { "managed_zone_id": 1, "managed_zone_name": "Khu A", "department_name": "..." }
  }
}
```
> **Lưu ý:** Chỉ trả về block `*_details` tương ứng với `role` của user. Ví dụ: citizen chỉ có `citizen_details`, guard chỉ có `guard_details`.
...
```
> Trả về đầy đủ thông tin user kèm role-specific details (`citizen_details` / `guard_details` / `manager_details` tùy role).
  ...
  "data": [
    {
      ...
      "status": "approved",
      "current_status": "active | upcoming | expired | cancelled"
    }
  ]
}
```
> `current_status` là trạng thái computed: `active` (đang trong khung giờ), `upcoming` (chưa đến giờ), `expired` (quá giờ), `cancelled` (đã hủy).
...
Tạo mã OTP 6 số cho khách đột xuất (hiệu lực 3 phút).
...
Xem lịch sử ra vào của tất cả xe thuộc sở hữu của cư dân. Chỉ trả về logs gắn với xe cư dân (không bao gồm log khách vãng lai).
...
Lấy log gần nhất theo làn xe — dùng để render bảng Dashboard thời gian thực.

**Query Params (bắt buộc):** `?lane_id=MAIN-IN&limit=20`

> `lane_id` là bắt buộc. Thiếu `lane_id` trả về 400. `limit` tối đa 100, mặc định 20.
...
Thống kê lượt vào/ra và cảnh báo theo làn xe trong 24h gần nhất.

**Query Params (bắt buộc):** `?lane_id=MAIN-IN`

> `lane_id` là bắt buộc. Thiếu `lane_id` trả về 400.
...
```json
{
  ...
  "action_reason": "emergency_vehicle | shipper_delivery | ai_error | vip_guest | maintenance | other",
  "note": "...",
  "image_base64": "..."
}
```

> `action_type` hợp lệ: `open_barrier`, `close_barrier`, `log_event`.
...
```
> `log_id` là log gốc được cập nhật (không tạo log mới). `note` chứa biển số đã sửa.
...
```json
{
  "success": true,
  "data": [
    { "hour": 7, "total_entries": 45 },
    { "hour": 8, "total_entries": 120 }
  ]
}
```
...
```json
{
  "success": true,
  "data": [
    { "access_method": "ai_plate_recognition", "count": 260 },
    { "access_method": "ai_camera_otp",        "count": 42 },
    { "access_method": "ai_camera_qr",         "count": 8 },
    { "access_method": "manual_guard",         "count": 10 }
  ]
}
```
...
| `start_date` | ISO date | Từ ngày |
| `end_date` | ISO date | Đến ngày |
| `gate_id` | int | Lọc theo cổng |
| `access_method` | string | Lọc theo phương thức |
| `license_plate` | string | Tìm theo biển số |
| `is_granted` | boolean | Lọc granted/denied |
...
**Query Params:** `?actor_id=1&action_type=APPROVE_VEHICLE&from=...&to=...`
...
```json
{
  ...
  "data": [
    {
      "audit_id": 1,
      "actor": { "user_id": 5, "full_name": "Manager A" },
...
> `action_type` chỉ cho phép: `open_barrier`, `close_barrier` (không có `log_event`).
...
**Response 201:** `{ "success": true, "message": "Đã đăng ký khách thành công", "data": { "registration_id": 7, "status": "approved" } }`
...
Cập nhật thông tin user. Chỉ truyền các field cần thay đổi — các field không truyền giữ nguyên giá trị cũ.
...
> - `role_details` map tới bảng role-specific tương ứng với role hiện tại của user (`citizens` / `security_guards` / `managers`)
> - `password` nếu truyền phải ≥ 6 ký tự; nếu không truyền thì không đổi
> - Trả lỗi `404` nếu user không tồn tại
> - Trả lỗi `409` nếu `username` hoặc `phone_number` đã tồn tại
...
Xóa user khỏi hệ thống (hard delete). Tự động cascade xóa dữ liệu liên quan trong bảng role-specific (`citizens` / `security_guards` / `managers`).

> - Trả lỗi `400` nếu manager cố xóa tài khoản của chính mình
> - Trả lỗi `404` nếu user không tồn tại
...
{
  "success": true,
  "message": "Đã xóa user nguyen_van_b"
}
```
...
```json
{
  ...
  "data": {
    "action": "OPEN",
    "access_type": "resident | guest | anti_passback | unknown",
    "log_id": 42,
    "check_in_time": "2026-01-10T08:05:00Z",
    "plate_text": "51A-12345",
    "confidence_score": 0.97,
...
```json
{
  ...
  "data": {
    "action": "KEEP_CLOSED",
    "log_id": 43,
    "check_in_time": "2026-01-10T08:06:00Z",
...
Backend (:5000)              IoT Gateway (TestingMode.py)     Arduino (Serial)
     │                                  │                           │
     │◄──── socket.connect ─────────────│                           │
     │                                  │◄── CAR_ARRIVED ───────────│ (Lane A - Entry)
     │                                  │◄── CAR_EXITING ───────────│ (Lane B - Exit)
     │◄── HTTP POST /gates/check-in ────│                           │
     │◄── HTTP POST /gates/verify-camera-otp ──│ (nếu phát hiện OTP/QR)
     │                                  │                           │
     │──── scan_result event ──────────►│                           │
     │                                  │──── ENTRY_GO:Name\n ─────►│ (mở cổng vào)
     │                                  │──── EXIT_GO:Name\n  ─────►│ (mở cổng ra)
     ...
     │                                  │──── MANUAL_OPEN_IN:Name──►│ (nếu lane IN)
     │                                  │──── MANUAL_OPEN_OUT:Name─►│ (nếu lane OUT)
     │                                  │  hoặc FORCE_CLOSE\n       │
```
...
**Xử lý tại Gateway (phân biệt theo `lane_id`):**
- `OPEN` + `lane_id` chứa "OUT" → gửi Serial `MANUAL_OPEN_OUT:<operator_name>\n`
- `OPEN` + `lane_id` khác → gửi Serial `MANUAL_OPEN_IN:<operator_name>\n`
...
#### `scan_result`
Gateway gửi kết quả nhận diện sau khi nhận phản hồi từ Backend (check-in hoặc verify-camera-otp).

**Payload:**
```json
{
  "lane_id": "MAIN-IN | MAIN-OUT",
  "status": "success | fail",
  "plate": "51A-12345",
  "owner_name": "string",
  "vehicle_type": "car | motorbike | Emergency Guest | QR Code | ...",
  "access_type": "resident | guest | otp | anti_passback | unknown",
  "captured_image": "data:image/jpeg;base64,..."
}
```
> `status` = `success` khi action = OPEN, `fail` khi KEEP_CLOSED/ALARM.

#### `video_stream`
Gateway stream frame camera liên tục lên Backend (hỗ trợ 2 camera: Làn VÀO và Làn RA). **Backend tự động re-emit event này dưới tên `live_frame` đến Frontend.** Frontend phải lắng nghe event `live_frame`.

**Payload (Gateway → Backend):**
```json
{ "lane_id": "MAIN-IN | MAIN-OUT", "image": "data:image/jpeg;base64,..." }
```

#### `live_frame` *(Backend → Frontend)*
Backend re-emit `video_stream` nhận từ Gateway thành event `live_frame` đến tất cả client Frontend.
...
Khi Arduino gửi tín hiệu `CAR_ARRIVED` (cổng vào) hoặc `CAR_EXITING` (cổng ra) qua Serial, Gateway thực hiện:
1. **Quét QR Code** trước (OpenCV `QRCodeDetector`) → nếu phát hiện UUID → gọi `POST /api/v1/gates/verify-camera-otp` với `code_type: "qr_uuid"`
2. **Không có QR** → Gọi AI nhận diện biển số → phân loại kết quả:
   - Nếu chuỗi 6 ký tự thuần số → xử lý như **OTP** → gọi `POST /api/v1/gates/verify-camera-otp` với `code_type: "otp_6digit"`
   - Ngược lại → xử lý như **biển số xe** → gọi `POST /api/v1/gates/check-in`

Payload thực tế check-in:
```json
{
  "lane_id": "MAIN-IN | MAIN-OUT",
  "plate_text": "51A-12345",
...
Payload thực tế OTP/QR:
```json
{
  "lane_id": "MAIN-IN | MAIN-OUT",
  "token_data": "482917 | 550e8400-e29b-...",
  "code_type": "otp_6digit | qr_uuid",
  "image_base64": "data:image/jpeg;base64,..."
}
```
...
| 38 | `PATCH /api/v1/managers/users/:id` | ✅ | FR_MAN_07 |
| 39 | `DELETE /api/v1/managers/users/:id` | ✅ | FR_MAN_07 |
| 40 | `POST /api/v1/gates/check-in` | ✅ | UC-13, UC-14 |
| 41 | `POST /api/v1/gates/verify-camera-otp` | ✅ | UC-06 Auto |
| 42 | `GET /api/v1/gates/:gateId` | ✅ | Heartbeat |
| 43 | `PUT /api/v1/citizens/vehicles/:vehicleId` | ✅ | UC-01 |
| 44 | `DELETE /api/v1/citizens/vehicles/:vehicleId` | ✅ | UC-01 |

**Tổng: 44 endpoints — 44 ✅ đã implement · 0 ❌ còn thiếu**
