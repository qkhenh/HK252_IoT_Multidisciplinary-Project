...
```json
{
  ...
  "note": "Ghi chú bổ sung (bắt buộc khi reason = other)"
}
```
...
```json
{
  "gate_id": "uuid",
  "action": "OPEN | KEEP_CLOSED",
  "action_reason": "emergency_vehicle | vip_guest | maintenance | other",
  "note": "Mô tả bổ sung"
}
```
...
```json
{
  ...
  "gate_id": "uuid",
  "action": "OPEN",
  "message": "Lệnh mở cổng đã gửi thành công đến IoT Gateway."
}
```
...
{
  "lane_id": "MAIN-IN",
  "extracted_code": "482917",
  "code_type": "otp | qr_uuid",
  "confidence": 0.95,
  "image_snapshot": "base64_encoded_jpeg"
}
```
...
| # | Endpoint | Mô tả |
|---|----------|-------|
| 1 | `GET /api/v1/citizens/logs` | Lịch sử ra vào cá nhân (UC-04) |
| 2 | `POST /api/v1/citizens/qr-code` | Tạo mã QR định danh động (UC-15) |
| 3 | `POST /api/v1/guards/ai-corrections` | Guard báo cáo biển số AI sai (UC-08) |
| 4 | `POST /api/v1/guards/guests` | Guard đăng ký khách thay cư dân (UC-02) |
